...
#' @export
ulap_bor_ln <- function(x, ...) UseMethod("ulap_bor_ln")
...
#'
#' @seealso \code{\link{lap_bor_ln}}, \code{\link{ugau_bor_ln}}, \code{\link{print.ndmodel}}, \code{\link{summary.ndmodel}}, \code{\link{plot.ndmodel}}
#'
#' @name ulap_bor_ln
NULL
...
ulap_bor_ln.default <- function(x, y, level, mu = 0, b = 1, k = 1, order = levels(y), sortid = TRUE, ...){
  ...
  res <- list(xnoise = x,
              ...
              distr = distr,
              model = "Uneven-Laplace borderline noise",
              param = list(level = level, mu = mu, b = b, k = k, order = order, sortid = sortid),
              call = call
  )
  class(res) <- "ndmodel"
  return(res)
...
ulap_bor_ln.formula <- function(formula, data, ...){
  ...
  res <- ulap_bor_ln.default(x = x, y = y, ...)
  res$call <- match.call(expand.dots = TRUE)
  res$call[[1]] <- as.name("ulap_bor_ln")
