...
#' @export
asy_uni_ln <- function(x, ...) UseMethod("asy_uni_ln")
...
#'
#' @seealso \code{\link{maj_udir_ln}}, \code{\link{asy_def_ln}}, \code{\link{print.ndmodel}}, \code{\link{summary.ndmodel}}, \code{\link{plot.ndmodel}}
#'
#' @name asy_uni_ln
NULL
...
#' @export
#' @rdname asy_uni_ln
asy_uni_ln.default <- function(x, y, level, order = levels(y), sortid = TRUE, ...){
  ...
  if(length(level) != nlevels(y)){
    stop("length of \"level\" must be equal to the number of levels in \"y\"")
  }
  ...
  res <- list(xnoise = x,
              ...
              distr = distr,
              model = "Asymmetric uniform label noise",
...
asy_uni_ln.formula <- function(formula, data, ...){
  ...
  res <- asy_uni_ln.default(x = x, y = y, ...)
  res$call <- match.call(expand.dots = TRUE)
  res$call[[1]] <- as.name("asy_uni_ln")
