...
#' @export
symd_gimg_an <- function(x, ...) UseMethod("symd_gimg_an")
...
#'
#' @seealso \code{\link{unc_vgau_an}}, \code{\link{symd_rpix_an}}, \code{\link{print.ndmodel}}, \code{\link{summary.ndmodel}}, \code{\link{plot.ndmodel}}
#'
#' @name symd_gimg_an
NULL
...
#' @export
#' @rdname symd_gimg_an
symd_gimg_an.default <- function(x, y, level, sortid = TRUE, ...){
  ...
  if(level > 0){
    for(i in 1:length(idnall)){
      ii <- idnall[i]
      v <- as.double(unname(x[ii,]))
      ms <- mean(v)
      sds <- sd(v)
      nvalues <- rnorm(n = ncol(x), mean = ms, sd = sds)
      x[ii,] <- nvalues
    }
  }
  ...
  res <- list(xnoise = x,
              ...
              idclean = idx_clean,
              distr = distr,
              model = "Symmetric/dependent Gaussian-image attribute noise",
...
symd_gimg_an.formula <- function(formula, data, ...){
  ...
  res <- symd_gimg_an.default(x = x, y = y, ...)
  res$call <- match.call(expand.dots = TRUE)
  res$call[[1]] <- as.name("symd_gimg_an")
