...
#' @export
sym_gau_an <- function(x, ...) UseMethod("sym_gau_an")

#' Symmetric Gaussian attribute noise
#'
#' Introduction of \emph{Symmetric Gaussian attribute noise} into a classification dataset.
#'
#'\emph{Symmetric Gaussian attribute noise} corrupts (\code{level}·100)\% of the values of 
#' each attribute in the dataset. In order to corrupt an attribute \emph{A}, (\code{level}·100)\% of the
#' samples in the dataset are chosen. Then, their values for \emph{A} are corrupted adding a random value
#' that follows a Gaussian distribution of \emph{mean} = 0 and \emph{standard deviation} = (\emph{max}-\emph{min})·\code{k}, being
...
#' \strong{Analyzing the presence of noise in multi-class problems: alleviating its influence with the one-vs-one decomposition}. 
#' \emph{Knowledge and Information Systems}, 38(1):179-206, 2014.
...
#'
#' @seealso \code{\link{sym_int_an}}, \code{\link{symd_uni_an}}, \code{\link{print.ndmodel}}, \code{\link{summary.ndmodel}}, \code{\link{plot.ndmodel}}
#'
#' @name sym_gau_an
NULL
...
sym_gau_an.default <- function(x, y, level, k = 0.2, sortid = TRUE, ...){
  ...
  res <- list(xnoise = x,
              ...
              idclean = idx_clean,
              distr = distr,
              model = "Symmetric Gaussian attribute noise",
...
sym_gau_an.formula <- function(formula, data, ...){
  ...
  res <- sym_gau_an.default(x = x, y = y, ...)
  res$call <- match.call(expand.dots = TRUE)
  res$call[[1]] <- as.name("sym_gau_an")
