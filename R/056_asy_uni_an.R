...
#' @export
asy_uni_an <- function(x, ...) UseMethod("asy_uni_an")

#' Asymmetric uniform attribute noise
#'
#' Introduction of \emph{Asymmetric uniform attribute noise} into a classification dataset.
#'
#' \emph{Asymmetric uniform attribute noise} corrupts (\code{level}[i]·100)\% of the values for
#' each attribute \emph{A}[i] in the dataset. In order to corrupt an attribute \emph{A}[i], (\code{level}[i]·100)\% of the
#' samples in the dataset are chosen. Then, their values for \emph{A}[i] are replaced by random different ones between
...
#'
#' @seealso \code{\link{symd_gimg_an}}, \code{\link{unc_vgau_an}}, \code{\link{print.ndmodel}}, \code{\link{summary.ndmodel}}, \code{\link{plot.ndmodel}}
#'
#' @name asy_uni_an
NULL
...
asy_uni_an.default <- function(x, y, level, sortid = TRUE, ...){
  ...
  for(a in 1:ncol(x)){
    ...
    if(num_noise[a] > 0){
      ...
      if(is.integer(x[,a])){
        newvalues <- sample_replace(x = min(x[,a]):max(x[,a]), size = num_noise[a], original = FALSE, ref = x[idx_noise[[a]],a])
      }
      else if(is.double(x[,a])){
        newvalues <- runif_replace(n = num_noise[a], min = min(x[,a]), max = max(x[,a]), original = FALSE, ref = x[idx_noise[[a]],a])
      }
  ...
  res <- list(xnoise = x,
              ...
              idclean = idx_clean,
              distr = distr,
              model = "Asymmetric uniform attribute noise",
...
asy_uni_an.formula <- function(formula, data, ...){
  ...
  res <- asy_uni_an.default(x = x, y = y, ...)
  res$call <- match.call(expand.dots = TRUE)
  res$call[[1]] <- as.name("asy_uni_an")
