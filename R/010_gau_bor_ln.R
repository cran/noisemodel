...
#' @export
gau_bor_ln <- function(x, ...) UseMethod("gau_bor_ln")

#' Gaussian borderline label noise
#'
#' Introduction of \emph{Gaussian borderline label noise} into a classification dataset.
#'
#' \emph{Gaussian borderline label noise} uses an SVM to induce the decision border 
...
#'
#' @seealso \code{\link{sigb_uni_ln}}, \code{\link{larm_uni_ln}}, \code{\link{print.ndmodel}}, \code{\link{summary.ndmodel}}, \code{\link{plot.ndmodel}}
#'
#' @name gau_bor_ln
NULL
...
gau_bor_ln.default <- function(x, y, level, mean = 0, sd = 1, k = 1, sortid = TRUE, ...){
 ...
 for(i in 1:nrow(x)){
  ...
  res <- list(xnoise = x,
              ...
              distr = distr,
              model = "Gaussian borderline label noise",
              param = list(level = level, mean = mean, sd = sd, k = k, sortid = sortid),
              call = call
  )
  class(res) <- "ndmodel"
  return(res)
...
gau_bor_ln.formula <- function(formula, data, ...){
  ...
  res <- gau_bor_ln.default(x = x, y = y, ...)
  res$call <- match.call(expand.dots = TRUE)
  res$call[[1]] <- as.name("gau_bor_ln")
