...
#' @export
asy_int_an <- function(x, ...) UseMethod("asy_int_an")
...
#'
#' @seealso \code{\link{asy_uni_an}}, \code{\link{symd_gimg_an}}, \code{\link{print.ndmodel}}, \code{\link{summary.ndmodel}}, \code{\link{plot.ndmodel}}
#'
#' @name asy_int_an
NULL
...
asy_int_an.default <- function(x, y, level, nbins = 10, sortid = TRUE, ...){
  ...
  res <- list(xnoise = x,
              ...
              idclean = idx_clean,
              distr = distr,
              model = "Asymmetric interval-based attribute noise",
...
asy_int_an.formula <- function(formula, data, ...){
  ...
  res <- asy_int_an.default(x = x, y = y, ...)
  res$call <- match.call(expand.dots = TRUE)
  res$call[[1]] <- as.name("asy_int_an")
