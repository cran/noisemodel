...
#' @export
exp_bor_ln <- function(x, ...) UseMethod("exp_bor_ln")

#' Exponential borderline label noise
#'
#' Introduction of \emph{Exponential borderline label noise} into a classification dataset.
#'
#' \emph{Exponential borderline label noise} uses an SVM to induce the decision border 
#' in the dataset. For each sample, its distance
#' to the decision border is computed. Then, an exponential distribution with parameter \code{rate} is used to compute the
...
#' @param rate a double with the rate for the exponential distribution (default: 1).
...
#' @references
#' J. Bootkrajang. 
#' \strong{A generalised label noise model for classification in the presence of annotation errors}. 
#' \emph{Neurocomputing}, 192:61–71, 2016. 
#' \doi{10.1016/j.neucom.2015.12.106}.
#'
#' @examples
#' # load the dataset
#' data(iris2D)
...
#'
#' @seealso \code{\link{pmd_con_ln}}, \code{\link{clu_vot_ln}}, \code{\link{print.ndmodel}}, \code{\link{summary.ndmodel}}, \code{\link{plot.ndmodel}}
#'
#' @name exp_bor_ln
NULL
...
exp_bor_ln.default <- function(x, y, level, rate = 1, k = 1, sortid = TRUE, ...){
  ...
  for(i in 1:nrow(x)){
    denfun[i] <- dexp(x = mindist[i], rate = rate)
  }
  ...
  res <- list(xnoise = x,
              ...
              distr = distr,
              model = "Exponential borderline label noise",
              param = list(level = level, rate = rate, k = k, sortid = sortid),
              call = call
  )
  class(res) <- "ndmodel"
  return(res)
...
exp_bor_ln.formula <- function(formula, data, ...){
  ...
  res <- exp_bor_ln.default(x = x, y = y, ...)
  res$call <- match.call(expand.dots = TRUE)
  res$call[[1]] <- as.name("exp_bor_ln")
